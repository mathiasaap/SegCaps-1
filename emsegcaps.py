...
def SegCapsEM(input_shape, modalities=1, n_class=2):
    ...
    conv1 = layers.Conv2D(filters=32, kernel_size=4, strides=1, padding='same', activation='relu', name='conv1')(x)
    ...
    segmentation_caps = ConvCapsuleLayer(kernel_size=3, 
                                num_capsule=n_class, 
                                strides=1, 
                                padding='same',
                                name='segmentation_caps')(deconv_caps_3_2)
    
    
    #print(deconv_caps_3_2.get_shape())
    #assert False
    out_caps = OutputCapsuleLayer(out_classes=n_class,
                                  name='out_seg')(segmentation_caps)
